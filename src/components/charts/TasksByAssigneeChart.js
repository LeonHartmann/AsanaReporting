...
export default function TasksByAssigneeChart({ tasks, onClick, isFullscreen }) {
  ...
  const maxItems = shouldShowAll ? sortedAssignees.length : (isFullscreen ? 25 : 10);
  const truncatedData = sortedAssignees.slice(0, maxItems);
  
  // If we truncated the data and we're not showing all, add an "Others" category
  let labels = [];
  let dataCounts = [];
  
  if (!shouldShowAll && sortedAssignees.length > maxItems) {
    // Get the top items
    labels = truncatedData.map(([assignee]) => assignee);
    dataCounts = truncatedData.map(([, count]) => count);
    
    // Add "Others" category with the sum of the remaining items
    const othersSum = sortedAssignees.slice(maxItems).reduce((sum, [, count]) => sum + count, 0);
    if (othersSum > 0) {
      labels.push('Others');
  ...
  return (
    ...
    >
      ...
      {isFullscreen && !showAllInFullscreen && sortedAssignees.length > maxItems && (
        <div className="text-center mt-4 mb-2">
          <p className="text-sm text-gray-500 dark:text-gray-400">
            Showing top {maxItems} of {sortedAssignees.length} assignees.
          </p>
          <button
      ...
      {!isFullscreen && sortedAssignees.length > maxItems && (
        <div className="text-xs text-gray-500 dark:text-gray-400 mt-2 text-center">
          Showing top {maxItems} of {sortedAssignees.length} assignees. Click to view more.
        </div>
      )}
      
      {/* Show a reset button if displaying all assignees */}
