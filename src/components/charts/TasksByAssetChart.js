...
export default function TasksByAssetChart({ tasks, onClick, isFullscreen }) {
  ...
  const maxItems = shouldShowAll ? sortedAssets.length : (isFullscreen ? 20 : 10);
  const truncatedData = sortedAssets.slice(0, maxItems);
  
  // If we truncated the data and we're not showing all, add an "Others" category
  let labels = [];
  let dataCounts = [];
  
  if (!shouldShowAll && sortedAssets.length > maxItems) {
    // Get the top items
    ...
    const othersSum = sortedAssets.slice(maxItems).reduce((sum, [, count]) => sum + count, 0);
    if (othersSum > 0) {
      labels.push('Others');
  ...
  return (
    ...
    >
      ...
      {isFullscreen && !showAllInFullscreen && sortedAssets.length > maxItems && (
        <div className="text-center mt-4 mb-2">
          <p className="text-sm text-gray-500 dark:text-gray-400">
            Showing top {maxItems} of {sortedAssets.length} asset types.
          </p>
          <button
      ...
      {!isFullscreen && sortedAssets.length > maxItems && (
        <div className="text-xs text-gray-500 dark:text-gray-400 mt-2 text-center">
          Showing top {maxItems} of {sortedAssets.length} asset types. Click to view more.
        </div>
      )}
      
      {/* Show a reset button if displaying all assets */}
